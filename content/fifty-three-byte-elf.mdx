...
<Terminal title="server.c — the harness" code={`int fd = memfd_create("golf", 0);
...
<Terminal title="fs/binfmt_elf.c — the gates that matter" code={`if (elf_ex->e_phentsize != sizeof(struct elf_phdr))
    goto out;
if (elf_ex->e_phnum < 1 ||
...
<Terminal title="naïve, 11 bytes — too many" code={`xor eax, eax        ; 31 c0    (2 bytes)
...
<Terminal title="payload at offset 32 — 9 bytes" code={`mov al, 11          ; b0 0b           (2 bytes) — execve syscall number
...
<Terminal title="tiny.asm — 53 bytes total" code={`bits 32
org 0x00010000
